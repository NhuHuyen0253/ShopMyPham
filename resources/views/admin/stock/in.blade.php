...
<div class="p-6 max-w-xl">
  ...
  <form method="POST" action="{{ route('admin.stock.in.store') }}" class="space-y-3">
    @csrf
    ...
    <select name="supplier_id" class="w-full border p-2">
      <option value="">-- Nhà cung cấp (optional) --</option>
    ...
    <input type="number" step="0.01" name="unit_cost" class="w-full border p-2" placeholder="Giá vốn/đv (tuỳ chọn)">
    ...
    <button class="px-4 py-2 bg-pink-600 text-white rounded">Lưu nhập kho</button>
  </form>
</div>
@endsection
