...
<div class="p-4 admin-page">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="admin-page-header mb-4">
            <div>
                <h1 class="admin-page-title">Sửa tồn kho</h1>
            </div>
        ...
        <div class="admin-card">
            <div class="admin-card-body">
                <form method="POST" action="{{ route('admin.stock_levels.update', $stockLevel) }}" class="space-y-4">
                    @csrf
                    @method('PUT')
                        ...
                        <select name="product_id" id="product_id" class="admin-select" required>
                            ...
                            @foreach($products as $product)
                                <option value="{{ $product->id }}"
                                    {{ old('product_id', $stockLevel->product_id) == $product->id ? 'selected' : '' }}>
                                    {{ $product->name }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="warehouse_id" id="warehouse_id" class="admin-select" required>
                            ...
                            @foreach($warehouses as $warehouse)
                                <option value="{{ $warehouse->id }}"
                                    {{ old('warehouse_id', $stockLevel->warehouse_id) == $warehouse->id ? 'selected' : '' }}>
                                    {{ $warehouse->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="admin-input"
                            min="0"
                            value="{{ old('quantity', $stockLevel->quantity) }}"
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="admin-input"
                            min="0"
                            value="{{ old('reorder_point', $stockLevel->reorder_point) }}"
                            placeholder="Ví dụ: 5"
                        >
                    </div>
                    ...
                    <div class="flex flex-wrap gap-3 pt-2">
                        <button type="submit" class="btn-admin-pink">
                            <i class="fas fa-save me-2"></i>Cập nhật tồn kho
                        </button>
